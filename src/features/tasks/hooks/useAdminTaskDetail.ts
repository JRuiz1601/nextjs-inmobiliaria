...
import type { Task, UpdateTaskByAdminDto } from "../types";
...
export function useAdminTaskDetail(taskId: string) {
  ...
  const [formData, setFormData] = useState<UpdateTaskByAdminDto>({
    title: "",
    description: "",
    dueDate: null,
  ...
  useEffect(() => {
    const load = async () => {
      try {
        setIsLoading(true);
        const data = await taskService.getForAdminById(taskId);
        setTask(data);
        setFormData({
          title: data.title,
  ...
  const handleSubmit = async (e?: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const updated = await taskService.updateForAdmin(taskId, formData);
      setTask(updated);
      setIsSaving(false);
      return updated;
    } catch (err) {
  ...
  const handleDelete = async () => {
    ...
    setIsDeleting(true);
    try {
      await taskService.removeForAdmin(taskId);
